...
===========================================================
Futuristic 72-hour AQI telemetry & ML forecasting dashboard
for 5 cities in Sindh, Pakistan (Permanent Dark Mode).

Features:
  - Futuristic Dark Mode Telemetry Aesthetic
  - Single 3-Day Average Hero Gauge + Daily Breakdown Cards
  - Pure Machine Learning Forecasting (RandomForest Model v5)
  - Interactive Dark Cartography & SHAP Telemetry
...
AQI_THEME = {
    "Good": {"color": "#10b981", "bg": "rgba(16,185,129,0.18)", "border": "rgba(16,185,129,0.45)", "text": "#34d399"},
    "Moderate": {"color": "#f59e0b", "bg": "rgba(245,158,11,0.18)", "border": "rgba(245,158,11,0.45)", "text": "#fbbf24"},
    "Unhealthy for Sensitive Groups": {"color": "#f97316", "bg": "rgba(249,115,22,0.18)", "border": "rgba(249,115,22,0.45)", "text": "#fb923c"},
    "Unhealthy": {"color": "#ef4444", "bg": "rgba(239,68,68,0.18)", "border": "rgba(239,68,68,0.45)", "text": "#f87171"},
    "Very Unhealthy": {"color": "#a855f7", "bg": "rgba(168,85,247,0.18)", "border": "rgba(168,85,247,0.45)", "text": "#c084fc"},
    "Hazardous": {"color": "#e11d48", "bg": "rgba(225,29,72,0.22)", "border": "rgba(225,29,72,0.5)", "text": "#fda4af"},
}
...
AQI_HEALTH_MESSAGES = {
    ...
    "Unhealthy for Sensitive Groups": "Elevated particulate density. Respiratory precautions recommended for sensitive groups. ⚠️",
    "Unhealthy": "Unhealthy atmospheric envelope. Wear masks and restrict prolonged outdoor exertion. 🚨",
...
<style>
    ...
    body, .stApp {
        ...
        background-image: 
            ...
            radial-gradient(at 85% 85%, rgba(168, 85, 247, 0.06) 0px, transparent 50%) !important;
    ...
    /* Hero Header */
    .hero-header {
        background: linear-gradient(135deg, #0b1528 0%, #112240 50%, #0369a1 100%);
    ...
    div[data-testid="stPills"] button {
        ...
        color: #94a3b8 !important;
        border: 1.5px solid #1e3256 !important;
...
def render_3day_trajectory(forecasts, city_name):
    ...
    fig.update_layout(
        ...
        paper_bgcolor='rgba(0,0,0,0)',
        yaxis=dict(gridcolor="#1e293b", range=[0, max(max(aqis) * 1.35, 170)], tickfont=dict(color="#f8fafc")),
...
def render_city_forecast_map(city_forecasts_dict):
    """Create an interactive dark-themed map showing 3-day forecast markers for all cities."""
...
def main():
    ...
    <div class="top-hud-bar">
        ...
        <div style="font-family: 'SF Mono', monospace; font-size: 0.74rem; color: #64748b;">
            AUTO-SYNC: HOURLY
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_left:
        ...
        st.markdown(f"""
        <div class="card-box" style="border-top: 4px solid {avg_theme['color']}; text-align: center;">
            <div>
            ...
            if fc:
                ...
                with map_cols[i]:
                    ...
                    <div style="text-align:center; padding: 1rem 0.6rem; border-radius: 18px;
                                background: {t['bg']}; border: 1.5px solid {t['border']}; box-shadow: 0 4px 15px rgba(0,0,0,0.2);">
    ...
    st.markdown("---")
    st.markdown("""
    <div style="text-align: center; color: #64748b; font-size: 0.84rem; padding: 0.8rem;">
